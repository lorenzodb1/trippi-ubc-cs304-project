...
class Location {
    ...
    public function getLocationIDWithParams( $city, $country ) {
      $query = "SELECT `locationID` FROM `location` WHERE `city`=" . ModelsUtils::mysqlstring( $city ) . " AND `country`=" . ModelsUtils::mysqlstring( $country ) . "";
    ...
    public function addLocation($id, $city, $country ) {
      ...
      $query = "INSERT INTO `location`(`locationID`, `city`, `country`) VALUES (" . ModelsUtils::mysqlstring( $id ) . "," . ModelsUtils::mysqlstring( $city ) . "," . ModelsUtils::mysqlstring( $country ) .")";
      
      return $this->submitQuery( $query );
    }

    public function doesDurationExist( $startDate, $endDate ) {
      $query = "SELECT * FROM `travelling_duration` WHERE `startDate`=" . ModelsUtils::mysqlstring( $startDate ) . " AND `endDate`=" . ModelsUtils::mysqlstring( $endDate ) . "";
    
      return $this->returnResult( $this->submitQuery( $query ) );
    }

    public function addDuration( $startDate, $endDate, $duration ) {
      $query = "INSERT INTO `travelling_duration`(`startDate`, `endDate`, `duration`) VALUES (" . ModelsUtils::mysqlstring( $startDate ) . ", " . ModelsUtils::mysqlstring( $endDate ) . "," . ModelsUtils::mysqlstring( $duration ) . ")";
